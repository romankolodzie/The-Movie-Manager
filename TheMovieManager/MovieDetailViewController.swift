        ...
        if let movie = movie {
            ...
            TMDBClient.sharedInstance().getFavoriteMovies { (movies, error) in
                if let movies = movies {
                    ...
                    performUIUpdatesOnMain {
                        if self.isFavorite {
                            self.toggleFavoriteButton.tintColor = TMDBClient.UI.orange
                        } else {
                            self.toggleFavoriteButton.tintColor = UIColor.lightGrayColor()
                        }
                    }
                } else {
                    print(error)
            ...
            TMDBClient.sharedInstance().getWatchlistMovies { (movies, error) in
                if let movies = movies {
                    ...
                    performUIUpdatesOnMain {
                        if self.isWatchlist {
                            self.toggleWatchlistButton.tintColor = TMDBClient.UI.orange
                        } else {
                            self.toggleWatchlistButton.tintColor = UIColor.lightGrayColor()
                        }
                    }
                } else {
                    print(error)
        ...
        TMDBClient.sharedInstance().postToFavorites(movie!, favorite: shouldFavorite) { (statusCode, error) in
            if let error = error {
                ...
                if statusCode == 1 || statusCode == 12 || statusCode == 13 {
                    ...
                    performUIUpdatesOnMain {
                        self.toggleFavoriteButton.tintColor = (shouldFavorite) ? TMDBClient.UI.orange: UIColor.blackColor()
                    }
                } else {
        ...
        TMDBClient.sharedInstance().postToWatchlist(movie!, watchlist: shouldWatchlist) { (statusCode, error) in
            if let error = error {
                ...
                if statusCode == 1 || statusCode == 12 || statusCode == 13 {
                    ...
                    performUIUpdatesOnMain {
                        self.toggleWatchlistButton.tintColor = (shouldWatchlist) ? TMDBClient.UI.orange : UIColor.blackColor()
                    }
                } else {
                    print("Unexpected status code \(statusCode)")
                }
